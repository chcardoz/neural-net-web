    ...
    this.generate = function () {
        if (layout_iterations < this.max_iterations && temperature > 0.000001) {
            ...
            for (i = 0; i < nodes_length; i++) {
                ...
                node_v.layout.tmp_pos =
                    node_v.layout.tmp_pos ||
                    new Vector3().setVector(node_v.position);
                ...
                for (j = i + 1; j < nodes_length; j++) {
                    ...
                    if (i != j) {
                        ...
                        node_u.layout.tmp_pos =
                            node_u.layout.tmp_pos ||
                            new Vector3().setVector(node_u.position);

                        delta = node_v.layout.tmp_pos
                            .clone()
                            .sub(node_u.layout.tmp_pos);
                        delta_length = Math.max(
                            EPSILON,
                            Math.sqrt(delta.clone().multiply(delta).sum())
                        );
                        ...
                        change = delta
                            .clone()
                            .multiply(
                                new Vector3().setScalar(force / delta_length)
                            );
            ...
            for (i = 0; i < edges_length; i++) {
                ...
                delta = edge.source.layout.tmp_pos
                    .clone()
                    .sub(edge.target.layout.tmp_pos);
                delta_length = Math.max(
                    EPSILON,
                    Math.sqrt(delta.clone().multiply(delta).sum())
                );
                ...
                change = delta
                    .clone()
                    .multiply(new Vector3().setScalar(force / delta_length));
            ...
            for (i = 0; i < nodes_length; i++) {
                ...
                delta_length = Math.max(
                    ...
                    Math.sqrt(
                        node.layout.offset
                            .clone()
                            .multiply(node.layout.offset)
                            .sum()
                    )
                );

                node.layout.tmp_pos.add(
                    node.layout.offset
                        .clone()
                        .multiply(
                            new Vector3().setScalar(
                                Math.min(delta_length, temperature) /
                                    delta_length
                            )
                        )
                );
                ...
                tmpPosition
                    .sub(node.layout.tmp_pos)
                    .divide(new Vector3().setScalar(10));
            ...
            if (!this.finished) {
                console.log(
                    "Average time: " + mean_time / layout_iterations + " ms"
                );
            }
            this.finished = true;
            return false;
